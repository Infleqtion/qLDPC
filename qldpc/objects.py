...
class CayleyComplex:
    ...
    is to say (for example) that if a ∈ A, then a^-1 ∈ A.
    ...
    def __init__(
        ...
        *,
        ...
        # check that the provided genreating sets are symmetric
        ...
        # identify the group generated by the provided (sub)sets, and verify TNC if applicable
        ...
        if bipartite and not CayleyComplex.satisfies_total_no_conjugacy(group, subset_a, subset_b):
            message = (
                "Provided group and subsets do not satisfy Total No Conjugacy.\n"
                + "Try again with `bipartite=False`."
            )
            raise ValueError(message)
        ...
        # typing for vertices of the Cayley complex
        vertex_type = tuple[abstract.GroupMember, int] | tuple[abstract.GroupMember, int, int]
        v_g: vertex_type
        v_ag: vertex_type
        v_gb: vertex_type
        v_agb: vertex_type

        # build associated subgraphs that are used to define a quantum Tanner code
        subgraph_0 = nx.DiGraph()
        subgraph_1 = nx.DiGraph()
        for gg, aa, bb in itertools.product(group.generate(), subset_a, subset_b):
            ...
            face = tuple(sorted([v_g, v_ag, v_gb, v_agb]))
            subgraph_0.add_edge(v_g, face, sort=(aa, bb))
            subgraph_0.add_edge(v_agb, face, sort=(aa, bb))
            subgraph_1.add_edge(v_ag, face, sort=(aa, bb))
            subgraph_1.add_edge(v_gb, face, sort=(aa, bb))

        # trim subgraphs to their smallest connected components
        joint_graph = nx.compose(subgraph_0, subgraph_1)
        nodes = min(nx.weakly_connected_components(joint_graph), key=len)
        self.subgraph_0 = subgraph_0.subgraph(nodes)
        self.subgraph_1 = subgraph_1.subgraph(nodes)
