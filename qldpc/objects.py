...
class CayleyComplex:
    ...
    def __init__(
        ...
        *,
        ...
        # take the double cover(s) of the group, if necessary
        ...
        self._assert_valid_generating_data()
    ...
    def _assert_valid_generating_data(self) -> None:
        for subset, name in [(self.subset_a, "subset_a"), (self.subset_b, "subset_b")]:
        ...
        if self.rank < 2 and not CayleyComplex.satisfies_total_no_conjugacy(
        ...
        if self.rank == 0:
            graph_a, graph_b = CayleyComplex.get_cayley_graphs(
                self.group, self.subset_a, self.subset_b
            )
            for graph, tag in [(graph_a, "A"), (graph_b, "B")]:
                if not nx.is_bipartite(graph):
                    raise ValueError(f"Cayley graph (G;{tag}) is not bipartite")
