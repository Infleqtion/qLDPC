...
def get_transversal_automorphism_group(
    ...
    *,
    ...
    group_aut_gens = _sympy_group_intersection_generators(
        group_code.to_sympy(), group_gates.to_sympy()
    )
    return abstract.Group(*map(abstract.GroupMember, group_aut_gens))


def _intersection_group_cache(
    group_a: sympy.PermutationGroup, group_b: sympy.PermutationGroup
) -> Hashable:
    """Cache the arguments to _sympy_group_intersection_generators."""
    return (
        tuple(tuple(generator) for generator in group_a.generators),
        tuple(tuple(generator) for generator in group_b.generators),
    )


@cache.use_disk_cache(CACHE_NAME, key_func=_intersection_group_cache)
def _sympy_group_intersection_generators(
    group_a: sympy.PermutationGroup, group_b: sympy.PermutationGroup
) -> tuple[tuple[int, ...], ...]:
    """Get the generators of the intersection of two Sympy permutation groups."""
    group = group_a.subgroup_search(group_b.contains)
    return tuple(tuple(generator) for generator in group.generators)
