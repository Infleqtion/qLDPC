...
class BBCode(QCCode):
    ...
    def get_equivalent_toric_layout_code_data(
        ...
        """Get the generating data for equivalent BBCodes with toric-code-like layouts.

        For simplicity, we consider BBCodes for qubits (with base field F_2) in the text below.

        A BBCode with a toric-code-like layout is generated by polynomials that look like
            poly_a = 1 + g + ..., and
            poly_b = 1 + h + ...,
        where g and h are generators of cyclic groups that may have different orders than those of
        the original BBCode.  To find equivalent toric-code-like BBCodes, we take
            poly_a = sum_j A_j --> poly_a / A_k = 1 + sum_{j != k} A_j / A_k, and
            poly_b = sum_j B_j --> poly_b / B_l = 1 + sum_{j != l} B_j / B_l.
        The each term in the sums over j != k and j != l are then candidates for g and h.
        This modification of polynomials and change-of-basis to g and h produces an equivalent
        BBCode so long as the conditions in Lemma 4 of arXiv:2308.07915 are satisfied.
        """
        if not nx.is_weakly_connected(self.graph):
            # a connected tanner graph is required for a toric-code-like layout to exist
            return []
        ...
        # identify pairs of monomials that can be combined to obtain a toric-code-like layout
        toric_params = []
        ...
        for a_1, a_2, b_1, b_2 in toric_params:
            ...
            gen_h = b_2 / b_1
            orders = (
                int(self.to_group_member(gen_g).order()),
                int(self.to_group_member(gen_h).order()),
            )
            ...
            We want to "change basis" from generators (x, y) to generators (g, h), where
                g = x^p y^q,
                h = x^u y^v.
            To do so, we identify exponnents (i, j, a, b) for which
                x^i y^j = g^a h^b,
            or equivalently
            ...
            for aa, bb in np.ndindex(orders):
                ...
                if (ii, jj) == (1, 0):
                    ...
                    gy, hy = aa, bb
            val_x = symbol_g**gx * symbol_h**hx
            val_y = symbol_g**gy * symbol_h**hy

            # build the new toric-code-like polynomials
            new_polys = []
            for new_poly in [(self.poly_a / a_1), (self.poly_b / b_1)]:
                new_poly = new_poly.expand().subs({self.symbols[0]: val_x, self.symbols[1]: val_y})

                # replace the "new" symbols by the "original" ones
